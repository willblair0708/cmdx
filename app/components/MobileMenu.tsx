...
const MobileMenu = memo(({ toggleMenu, navItems }: MobileMenuProps) => (
  <AnimatePresence>
    <motion.div
      initial="closed"
      animate="open"
    ...
    >
      ...
      >
        <motion.div
          ...
          whileTap={{ scale: 0.95 }}
        >
          <GuestOSIcon size={85.5} className="mt-6 text-black" color="black" />
        </motion.div>
        ...
        >
          ...
          >
            <div className="w-full gap-[20px] pl-[140px]">
              {navItems.map((item) => (
                <MobileMenuItem key={item.text} item={item} toggleMenu={toggleMenu} />
              ))}
            </div>
          </motion.div>
          ...
          >
            {/* Changed contact link to an in-page anchor */}
...
const MobileMenuItem = memo(
  ({ item, toggleMenu }: { item: { text: string; href: string }; toggleMenu: () => void }) => {
    // Instead of using an external pathname, track the hash locally:
